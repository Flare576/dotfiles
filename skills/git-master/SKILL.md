...
name: git-master
description: >
  MUST USE for ANY git operations: committing, rebasing, squashing, or searching
  git history. Use when the user says "commit", "make a commit", "rebase", "squash
  commits", "clean up history", "find when X was added", "who wrote this line",
  "when did this bug start", "git blame", "git bisect", or any git-related request.
  Handles commit style detection (semantic/plain/short), atomic commit planning,
  dependency ordering, fixup/autosquash workflows, interactive rebase, conflict
...
|---------------------|------|---------|
| Commit intent (e.g., "commit") | `COMMIT` | Phase 0-6 (existing) |
| Rebase/squash intent (e.g., "rebase") | `REBASE` | Phase R1-R4 |
| History lookup intent (e.g., "find when") | `HISTORY_SEARCH` | Phase H1-H3 |
...
**VENDORING EXCEPTION:**
When adding an external library or vendored directory as a single unit (e.g., cloning an
upstream skill, copying a package), ONE commit is appropriate IF:
- All files originate from a single external source
- The files are committed as-is (no mixing with your own changes)
- The commit message clearly states the vendor source (e.g., "vendor: add bmad-labs/atlassian-rest skill")

Mix your patches into a SEPARATE commit from the vendor import.
...
Every commit MUST include a Co-authored-by trailer identifying the AI agent that made it.
You are the agent. Sign with your own name.

```bash
git commit -m "{Commit Message}" -m "Co-authored-by: {YourAgentName} <{your-agent-name}@omp.local>"
```

Examples:
- Sisyphus: `Co-authored-by: Sisyphus <sisyphus@omp.local>`
- Beta: `Co-authored-by: Beta <beta@omp.local>`
- Atlas: `Co-authored-by: Atlas <atlas@omp.local>`
- Prometheus: `Co-authored-by: Prometheus <prometheus@omp.local>`

If you don't know your agent name, use: `Co-authored-by: OMP Agent <omp-agent@omp.local>`
</execution>

---

### Talisman Hook Failures

If `git commit` fails with a Talisman report:

1. Read the report — it lists flagged files and provides exact checksums.
2. For each flagged file, confirm it contains NO actual secrets (tokens, passwords, private keys).
   False positives include: sha512 integrity hashes in lockfiles, API documentation examples,
   env file name patterns (`.zshenv`, `.zshenv.*`), example keys like `PROJ-123`.
3. If confirmed false positive, add to `.talismanrc` in the repo root:
   ```yaml
   fileignoreconfig:
   - filename: path/to/flagged/file
     checksum: <checksum from talisman report>
   version: "1.0"
   ```
4. Stage `.talismanrc` and retry the commit.
5. If the file DOES contain real secrets: remove the secret, move it to `~/.doNotCommit.d/`, never retry.
...
"squash commits" intent (e.g., "cleanup")
  -> INTERACTIVE_SQUASH

"rebase on main" intent (e.g., "update branch")
  -> REBASE_ONTO_BASE

"autosquash" / "apply fixups"
  -> AUTOSQUASH

"reorder commits" intent
  -> INTERACTIVE_REORDER
